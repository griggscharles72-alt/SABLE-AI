...
def scan():
    report = {"status": "scan complete", "timestamp": utc_now(), "issues": [], "system_info": get_system_info()}
    write_json(STATE_PATH, report); write_json(OUTPUT_PATH, report); return report
def status():
    last = read_json(STATE_PATH, default={})
    return {"status": "operational", "last_scan_at": last.get("timestamp"), "has_report": bool(last)}
